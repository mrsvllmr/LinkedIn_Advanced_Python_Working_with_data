...
def filterNonEarthquakes(x):
    if x["properties"]["type"] != "earthquake":
        return True
    return False


earthquakes = list(filter(filterNonEarthquakes, data["features"]))
typesList = []
for quake in earthquakes:
    if quake["properties"]["type"] not in typesList:
        typesList.append(quake["properties"]["type"])
typesList.sort()
print(typesList)
